...
class HybridAsrWithIvectorDataset(NnetPytorchDataset):

    Minibatch = namedtuple('Minibatch', ['input', 'target', 'ivector', 'metadata'])
    ...
    def build_dataset(cls, ds):
        return HybridAsrWithIvectorDataset(
            ds['data'], ds['tgt'], ds['ivectors'],
    ...
    def __init__(self, datadir, targets, ivectors,
    ...
    def __getitem__(self, index):
        ...
        return HybridAsrWithIvectorDataset.Minibatch(x, target, ivector, metadata)
    ...
    def minibatch(self):
        ...
        return HybridAsrWithIvectorDataset.Minibatch(input_tensor, output_tensor, ivector_tensor, metadata) 
        

    def evaluation_batches(self):
        print (len(self.utt_subset))
        for u in self.utt_subset:
            ...
            for idx in range(start, end, self.chunk_width):
                ...
                if i == self.batchsize:
                    ...
                    yield HybridAsrWithIvectorDataset.Minibatch(input_tensor, output_tensor, ivector_tensor, metadata) 
                    i = 0
                    inputs, output, ivectors = [], [], []
                    name = []
    ...
    def move_to(b, device):
        ...
        '''
        return HybridAsrWithIvectorDataset.Minibatch(b.input.to(device), b.target.to(device), b.ivector.to(device), b.metadata)
