...
class HybridAsrWithIvectorAuxTgtDataset(NnetPytorchDataset):

    Minibatch = namedtuple('Minibatch', ['input', 'target', 'aux_target', 'ivector', 'metadata'])
    ...
    def __init__(self, datadir, targets, aux_targets, ivectors,
        ...
        # Load ivectors paths from scp file. We do not load the actual
        # ivectors here, since we will do that on-the-fly
        self.ivector_paths = load_ivector_paths(ivectors)
        # Get first ivector to obtain dimension
        self.ivectordim = ivector_dim
        ...
        # Get targets. Dummy targets for utterances where labels are unknown
        with open(aux_targets) as f:
            self.aux_targets = get_targets(f)
    ...
    def __getitem__(self, index):
        ...
        aux_target = self.aux_targets[utt_name][target_start: target_end]
        ...
        # Get the ivector
        ivector = load_ivector(self.ivector_paths[utt_name.decode()])
        ...
        return HybridAsrWithIvectorAuxTgtDataset.Minibatch(x, target, aux_target, ivector, metadata)
    ...
    def minibatch(self):
        ...
        aux_output = torch.zeros(
            (self.batchsize, self.subsample_chunk_width),
            dtype=torch.int64
        )
        ivector_tensor = torch.zeros((self.batchsize, self.ivectordim))
        name, split = [], []
        size = 0
        while size < self.batchsize:
            ...
            aux_output[size, :] =torch.LongTensor(sample.aux_target)
            ivector_tensor[size, :] = torch.from_numpy(sample.ivector)

            size += self.size(idx)
        
        output_tensor = torch.LongTensor(output)
        aux_output_tensor = torch.LongTensor(aux_output)
        self.closure(set(split))
        return HybridAsrWithIvectorAuxTgtDataset.Minibatch(input_tensor, output_tensor, 
            aux_output_tensor, ivector_tensor, metadata) 
    ...
    def evaluation_batches(self):
        for u in self.utt_subset:
            ...
            i = 0
            inputs, output, aux_output, ivectors = [], [], [], []
            name = []
            for idx in range(start, end, self.chunk_width):
                ...
                aux_output.extend(sample.aux_target)
                ivectors.append(sample.ivector)
                i += 1
                ...
                if i == self.batchsize:
                    ...
                    output_tensor = torch.LongTensor(output)
                    aux_output_tensor = torch.LongTensor(aux_output)
                    ivector_tensor = torch.tensor(ivectors, dtype=torch.float32)
                    yield HybridAsrWithIvectorAuxTgtDataset.Minibatch(input_tensor, output_tensor,
                        aux_output_tensor, ivector_tensor, metadata) 
                    i = 0
                    inputs, output, aux_output, ivectors = [], [], [], []
                    name = []
    ...
    def move_to(b, device):
        ...
        '''
        return HybridAsrWithIvectorAuxTgtDataset.Minibatch(b.input.to(device), b.target.to(device),
            b.aux_target.to(device), b.ivector.to(device), b.metadata)
