...
class HybridAsrWithAuxTgtDataset(NnetPytorchDataset):

    Minibatch = namedtuple('Minibatch', ['input', 'target', 'aux_target', 'metadata'])
    ...
    def build_dataset(cls, ds):
        return HybridAsrWithAuxTgtDataset(
            ds['data'], ds['tgt'], ds['aux_tgt'],
    ...
    def __init__(self, datadir, targets, aux_targets,
    ...
    def __getitem__(self, index):
        ...
        return HybridAsrWithAuxTgtDataset.Minibatch(x, target, aux_target, metadata)
    ...
    def minibatch(self):
        ...
        return HybridAsrWithAuxTgtDataset.Minibatch(input_tensor, output_tensor, 
            aux_output_tensor, metadata) 
        ...
        for u in self.utt_subset:
            ...
            for idx in range(start, end, self.chunk_width):
                ...
                if i == self.batchsize:
                    ...
                    output_tensor = torch.LongTensor(output)
                    aux_output_tensor = torch.LongTensor(aux_output)
                    yield HybridAsrWithAuxTgtDataset.Minibatch(input_tensor, output_tensor,
                        aux_output_tensor, metadata) 
                    i = 0
                    inputs, output, aux_output = [], [], []
                    name = []
    ...
    def move_to(b, device):
        ...
        '''
        return HybridAsrWithAuxTgtDataset.Minibatch(b.input.to(device), b.target.to(device),
            b.aux_target.to(device), b.metadata)
