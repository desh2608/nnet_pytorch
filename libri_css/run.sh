...
if [ $stage -le 3 ]; then
  ...
  if [[  $(hostname -f) ==  *.clsp.jhu.edu ]]; then
    fbank=$(basename fbankdir) # in case was absolute pathname (unlikely), get basename.
    utils/create_split_dir.pl /export/b{07,08,09,10}/$USER/kaldi-data/egs/libri_css/s5/$fbank/storage \
     $fbankdir/storage
  fi
...
if [ $stage -le 5 ]; then
  targets=data/${train_set}_fbank/pdfid.${subsampling}.tgt
  # ali-to-pdf ${tree}/final.mdl ark:"gunzip -c ${tree}/ali.*.gz |" ark,t:${targets}
  split_memmap_data.sh data/${train_set}_fbank $targets ${num_split}
fi
