...
class Segmentation(object):
    ...
    def __init__(self, region_type):
        ...
        if region_type == "speech":
            # We need all speech regions (single and overlap) so we will
            # convert all overlap frames to single frames in preprocessing
            region_type = "single"
            self.convert_overlap_to_single = True
        else:
            self.convert_overlap_to_single = False
    ...
    def initialize_segments(self, alignment, frame_shift=0.01):
        ...
        if self.convert_overlap_to_single:
            alignment = [2 if x == 3 else x for x in alignment]
