...
def main(args):
    ...
    with open(os.path.join(args.wavpath, "mixspec.json"), "r") as f:
        mixlog = json.load(f)
    
    if (args.order_by_overlap):
        mixlog = sorted(mixlog, key=lambda x: x["actual_overlap_time_ratio"])
    n = len(str(len(mixlog)))
        
    for i,recording in enumerate(mixlog):
        wav_name = recording["output"].split(os.path.sep)[-1]
        if args.order_by_overlap:
            reco = "libricss_sim_{}".format(str(i).zfill(n))
        else:
...
def make_argparse():
    ...
    parser.add_argument(
        "--order-by-overlap", action='store_true', help="Order the utterances by increasing overlap ratio"
    )
